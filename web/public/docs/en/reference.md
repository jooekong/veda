...
```json
{
  ...
  "data": {
    ...
    "truncated": false,
    "entries": [
      { "path": "/docs", "is_dir": true, "abstract": "design and deployment docs", "file_count": 42 },
      { "path": "/wiki", "is_dir": true, "file_count": 310 },
      { "path": "/README.md", "is_dir": false, "abstract": "project overview", "size_bytes": 4096 }
    ]
  }
}
```

- **One level only**, no recursion. To go deeper use `GET /v1/overview/{path}` or `POST /v1/search`.
- **Ordering is directories first, then files**, each group by path.
- `entries` is capped at **200**; beyond that `truncated: true`. Truncation follows the ordering above, so directories always survive. `stats` always describes the whole workspace, unaffected by truncation.
- `abstract` is **omitted entirely** (not `null`) when an entry has no L0 yet. `file_count` appears only on directories (recursive count), `size_bytes` only on files.
- `summary_state` is one of `ready` (every returned entry has a summary) / `partial` (coverage is incomplete — a statement of fact, **not a promise that retrying helps**: a directory that became empty has its summary deleted and will never get another) / `disabled` (server has no `[llm]`, so no new summaries will be produced; **abstracts already cached are still returned**, matching `/v1/abstract`).
- Always returns `200` regardless of summary state — it aggregates many summaries, so the 202/501 tri-state of `/v1/abstract` does not apply. With no summaries at all it degrades to "directory tree + counts", which is still useful.
- It is a **best-effort snapshot**: assembled from several independent reads, so under concurrent writes `entries`, `stats` and `file_count` may reflect slightly different moments. If any read fails the whole request returns `500` rather than a partial result.
...
- **Response**: `{ answer, citations, hit_count, estimated_context_tokens }`. Citations come in two kinds: **document citations** `{index, path, spans}` (`spans` are chunk ranges, **an empty array means the whole file**; two passages of one file yield two citations with the same path — group by path when displaying) and **memory citations** `{index, memory: {id, content, updated_by, updated_at}}` with no `path` — the workspace's team memories participate in retrieval as a second evidence source (see the Team memory section). Render by shape: `path` → file, `memory` → the memory line; older consumers that skip path-less entries degrade gracefully. When nothing supports an answer, a fixed refusal phrase comes back with empty citations (no fabrication).
...
### Team memory (`/v1/memory/*`)

The third asset class of an fs workspace: **one-line facts** (decisions, pitfalls, environment quirks, preferences) stored alongside documents. Two ownership domains — the **team domain** (`scope: "team"`: everyone in the workspace can read, edit, and hard-delete; every row is signed; wiki-style, no approval flow) and a **personal domain** (`scope: "mine"`: follows the key, physically invisible to others). Team memories automatically participate in `/v1/answer` retrieval and can come back as memory citations. Designed for agents (the MCP memory tools share these semantics); read-only `wk_` keys can search, writes need a readwrite key.

- `POST /v1/memory`: `{ content (≤4096 chars), scope? ("mine" default / "team"), kind? ("fact" default / "decision" / "procedure" / "preference"), topic?, source_ref?, expires_at? }`. Response `{ memory, duplicate, neighbors }` — `duplicate: true` means an identical line already exists in that domain (idempotent, the existing row comes back); `neighbors` are the domain's top-3 nearest memories — **when one is semantically close, PATCH it instead of saving another**.
- `GET /v1/memory/search?query=…&scope=&limit=`: semantic retrieval. Omitting `scope` searches team + this key's personal domain together.
- `GET /v1/memory/context?query=…&limit=`: the session-start call — one request for "memories relevant to what I'm about to do".
- `PATCH /v1/memory/{id}`: any of `{ content? / topic? / source_ref? / expires_at? }`; a content change re-embeds and re-signs (`updated_by`).
- `DELETE /v1/memory/{id}`: hard delete, immediately unretrievable. Cross-domain operations (touching someone else's personal memory) return `404` (existence is not leaked).
- Item shape: `{ id, scope: "team"|"mine", kind, content, topic?, origin_workspace_id?, source_ref?, expires_at?, created_by, created_at, updated_by, updated_at }`; retrieval responses add a `score`.
...
- **Auth**: the same gate as the REST data plane — `Authorization: Bearer wk_…` (fs workspace; db kind → 400). A read-only `wk_` runs all seven read-only tools plus memory retrieval (`memory_search`/`memory_context`); the memory-writing tools (`memory_save`/`memory_update`/`memory_delete`) need a readwrite key and return a readable refusal otherwise.
