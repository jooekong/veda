...
- `veda-sql`：VedaSqlEngine（DataFusion session 管理）；FilesTable（递归 dentry 枚举）；CollectionTable（Milvus 查询 → Arrow）；支持 SELECT/WHERE/COUNT/JOIN 等标准 SQL；8 个 mock 单元测试
