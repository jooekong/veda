...
基础层（types + core）、存储层（MySQL + Milvus）、Pipeline（embedding + chunking）全部实现，含单元测试和集成测试。
...
veda-sql        DataFusion SQL 引擎                 (空壳)
veda-server     Axum HTTP 层                        (空壳)
veda-cli        CLI 客户端                          (空壳)
...
- `veda-core`：MetadataStore/MetadataTx/VectorStore/TaskQueue/EmbeddingService trait 定义；FsService（write/read/list/stat/delete/copy/rename/mkdir，含 dedup/COW/分层存储/outbox）；SearchService（hybrid/semantic/fulltext 三模式）；path normalization；SHA256 checksum；19 个 mock 单元测试 + 12 个工具单元测试
- `veda-store`：MysqlStore（MetadataStore + MetadataTx + TaskQueue 实现，含 schema migration）；MilvusStore（VectorStore 实现，REST v2 client）；5 个 MySQL 集成测试 + 1 个 Milvus 集成测试
...
- 单元测试：`cargo test`（50 个测试，全部自动运行）
- 集成测试：`cargo test -- --ignored`（10 个测试，需要 `config/test.toml` 配置真实 MySQL/Milvus/Embedding 服务）
