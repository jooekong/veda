...
- **M3a 操作者身份 + 工号/部门域 + hybrid 检索（2026-08-18 实现，08-19 测试环境上线 + 08-20 冒烟）**：施工图归档 `docs/archive/plans/agent-memory-m3a.md`（Codex 对抗评审 7 findings 裁决入档；两拍板=无历史数据迁移、操作者头不加信任闸；测试环境跑 identity-only 模式——`[people]` 未配置，dept 域不可用、身份按入口分立；生产未部署，随下次发版窗口连同 M2a）。
  - **身份层**：`veda_principals` 拆为纯人表（`emp_no` UNIQUE 权威合并键 + `dept_id/dept_name/profile_synced_at` 目录缓存）+ 新表 `veda_principal_identities(source, external_id → principal_id)`；同一人多入口（企微 userid / 工号）经人员目录（`PersonDirectory` trait，HTTP 实现 `veda-server/src/people.rs`，config `[people]`，SSO 契约待接）懒查合并——**合并只有原地补全/attach 两种形态，emp_no 冲突保分立 warn 不重定向**（无孤儿路径）。目录降级：已知身份用缓存（≤24h TTL 懒刷新）；全新身份+目录故障=该请求按无操作者处理（读降团队域、写 mine/dept 报错，不造半身份）；`[people]` 未配置=identity-only 模式（按入口分立，无部门）。
  - **操作者透传**：`X-Veda-Operator: <source>:<external_id>`（source ∈ wecom|emp），wk_ 鉴权后即生效（公司内互信拍板，无 key 级信任闸），格式非法 400。`MemoryActor` 扩为 `{workspace_id, principal_id, dept_id?}`；tunnel **私聊带头/群聊不带**（受众规则：注入域 ⊆ 答案可见域，服务端零新参数）；MCP 经 /mcp 请求头、CLI 经 `VEDA_OPERATOR` env（代码先行未发版）。
