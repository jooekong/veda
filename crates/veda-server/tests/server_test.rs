...
async fn h_write(
    ...
    let ws = match resolve_ws(&st, auth_hdr(&headers)).await {
        Some(w) => w,
        None => return StatusCode::UNAUTHORIZED.into_response(),
    };
    match st
        .fs
        .write_file(&ws, &format!("/{p}"), &body, None, None)
        .await
    {
        ...
        match st.fs.list_dir(&ws, &path).await {
            Ok(entries) => {
                let v: Vec<serde_json::Value> = entries
                    .iter()
                    .map(|d| serde_json::json!({"name": d.name, "is_dir": d.is_dir}))
                    .collect();
...
async fn server_account_workspace_fs_flow() {
    ...
    // Write file
    let resp = c
        .put(format!("{base}/v1/fs/docs/hello.txt"))
    ...
    // Read file
    let resp = c
        .get(format!("{base}/v1/fs/docs/hello.txt"))
        .header("Authorization", format!("Bearer {wk}"))
        .send()
        .await
        .unwrap();
    ...
    // List directory
    let resp = c
        .get(format!("{base}/v1/fs/docs?list"))
        .header("Authorization", format!("Bearer {wk}"))
        .send()
        .await
        .unwrap();
    ...
    // Delete file
    let resp = c
        .delete(format!("{base}/v1/fs/docs/hello.txt"))
        .header("Authorization", format!("Bearer {wk}"))
        .send()
        .await
        .unwrap();
