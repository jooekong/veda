...
非目标（v1 不做）：
- 公网 SaaS endpoint（先用内网 IP `10.79.51.161:3000` 写死）
- 浏览器 SSO 登录
- 匿名账号自动注册（演进到 v2 再做）
...
| 决策点 | 结论 | 备注 |
|--------|------|------|
| 部署模式 | SaaS 默认（目标态） | 当前 endpoint 先写死 `10.79.51.161:3000` |
...
8. 打印下一步
   echo "✓ veda 已安装"
   echo "下一步："
   echo "  veda config set server_url http://10.79.51.161:3000"
...
Agent (skill 触发):
  ...
  Step 2: 装好后，跑 veda config show
    server_url 仍是 default localhost:3000 → 跑 veda config set server_url http://10.79.51.161:3000
