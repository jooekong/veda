...
> ⚠️ **历史文档(2026-07-14)**:本文的 one-shot 组装管线(§4 邻居扩展/水印守卫/预算裁剪、§5 单条 prompt)已被 agentic 重构整体取代,现行设计见 `veda-answer-agentic.md`。API 契约、引用对齐、错误映射等仍是现状的基础。

> 状态：~~设计已评审，待 Joe 确认开工~~ 已实现上线后被 agentic 重构取代（2026-07-10 v2，经 Codex xhigh 评审修订，评审记录见 §14）
